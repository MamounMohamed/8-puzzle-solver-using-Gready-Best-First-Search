...
class node {
    ...
    // the heuristic function we're using is calculating the Manhattan distance sum between the puzzle and the goal puzzle
    public int heuristic() {
        int heuristic = 0;
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                heuristic += rightSpot(i, j, puzzle[i][j]);
                // the heuristic is the sum of all the manhattan distances of every element of the puzzle and it's location on the goal array
            }
        }
        return heuristic;
    }
...
class Main {
    public static void GreedySearch(node root) { // we set goal = false , initialize the priority queue and the visited set , add the root them ;
        boolean goal = false;
        ...
        while (!priorityQueue.isEmpty() && !goal) {
            ...
            for (int i = 0; i < current.children.size(); i++) { // we iterate to the children of  the nodes
                ...
                node x = contains(priorityQueue, currentChild);//askes if priority queue contains the node currentchild (if yes it will return the node else it will return null)
                ...
                if (x != null & !contains(Visited, currentChild)) { // if currentchild is in the queue but not visited we see if we can decrease the cost of it or not
                    if (x.heuristic() > currentChild.heuristic() ) { //if x cost is bigger we remove it and add the currentChild to the queue
